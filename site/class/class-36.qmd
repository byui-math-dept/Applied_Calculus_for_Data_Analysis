...
1. Draws rectangles over the top of a given function.
1. The midpoint of top of each rectangle passes through the function. 
1. f - a function f(x)
1. a - left end of graph
1. b - right end of graph
1. num_rectangles - how many rectangles to plot.
1. method - One of "left", "right", or "mid".  Defaults to mid.
...
1. Start at half of dx to the right of a, and then step by dx.
xi <- seq(a+dx/2,b,dx)
...
1. Start a plus dx, and then step by dx.
xi <- seq(a+dx,b,dx)
points(xi,g(xi),pch=16,col=2)
segments(xi,rep(0,length(xi)),xi,g(xi),col=2)
...
1. Mid: Start at half of dx to the right of a, and then step by dx.
xi.m <- seq(a+dx/2,b,dx)
1. Left: Start a, and then step by dx.
xi.L <- seq(a,b-dx,dx)
1. Start a plus dx, and then step by dx.
xi.R <- seq(a+dx,b,dx)
