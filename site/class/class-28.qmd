...
```r
    1. If not done already, load the data. 
library(data4led)
bulb <- led_bulb(1,seed=123)
t <- bulb$hours
y <- bulb$percent_intensity
...
```r
    1. Let's create a function to solve this system of equations. 
solvesystem <- function(c11, c12,b1,c21,c22,b2){ 
  c((b1*c22 - c12*b2)/(c11*c22 - c21*c12),
    (c11*b2 - b1*c21)/(c11*c22 - c21*c12))
}
    1. Before moving forward, let's verify that the function works.
    1. The solution to $2x+3y=4$, $5x+6y=7$ is $(-1,2)$.  
    1. Does this function yield the same result?
solvesystem(2,3,4,5,6,7)

    1. Now we can solve the problem at hand. 
c11 <- 
c12 <- 
b1 <- 
c21 <- 
...
```r
    1. If not done already, you'll need to load the data. 
library(data4led)
bulb <- led_bulb(1,seed=123)
t <- bulb$hours
y <- bulb$percent_intensity

    1. If not done already, define the solvesystem function and find the critical values. 
solvesystem <- function(c11, c12,b1,c21,c22,b2){ 
  c((b1*c22 - c12*b2)/(c11*c22 - c21*c12),
    (c11*b2 - b1*c21)/(c11*c22 - c21*c12))
}
...
sol
    ...
    1. Define the model f2, and and create the plot. 
...
```r
1. Florida Tropical Storm Data (2000-2020) from Wikipedia
year <- seq(2000,2020,1)
storms <- c(4,4,8,8,6,8,2,8,8,4,8,6,4,3,3,4,5,7,4,7,13)

a <-
b <- 
1. You should get a=124 and b=144.5211. 
1. You'll need the factorial() when entering x! into R.
```
...
```r
rm(list=ls())
1. Florida Tropical Storm Data (2000-2020)
storms <- c(4,4,8,8,6,8,2,8,8,4,8,6,4,3,3,4,5,7,4,7,13)
