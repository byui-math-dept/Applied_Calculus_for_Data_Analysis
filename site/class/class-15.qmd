...
- Critique the following visual fits for $f_5$. 
    ...
    - https://byuimath.com/bmw/all/119/uploads/Class/f5-fit2.png
...
- Consider the 6 functions you have been fitting in https://byuistats.github.io/M119/project1.html. 
...
Use the storms listed on Wikipedia's https://tinyurl.com/51vyiwjw page to answer the questions below. Feel free to head to yesterday's page to grab code examples to help you work on the things below. 
...
1. The element of x must be a whole numbers.
  prod(p(x,lambda))
}

    1. #Possible Parameter Values###
lambda <- seq(0,10,0.001)

    1. #Data###
1. Florida Hurricane Data (2000-2022)
data <- c(4,4,8,8,6,8,2,8,8,4,8,6,4,3,2,4,6,7,4,7,13,3,3)

1. Here we calculate the output from the likelihood function given the observed data.
y <- sapply(lambda,FUN=LP,x=data)

1. We plot the likelihood function.
...
1. x and lambda must be positive
  lambda*exp(-lambda*x)
}

    1. #Define the Likelihood function###
LE <- function(lambda,x){
1. The elements of x must be positive.
  prod(f2(x,lambda))
}
...
lambda <- seq(0,10,0.001)
    ...
    1. #Data###
1. Some Simulated Data (This is data from an Exponential random variable.)
...
f3 <- function(x,mu=0,s=1){
  (1/sqrt(2*pi*s^2))*exp(-(x-mu)^2/(2*s^2))
    ...
    1. #Define the likelihood function###
1. For simplicity, we'll assume sigma is 1.
...
mu <- seq(-10,10,0.001)
    ...
    1. #Data###
1. Some more Simulated Data (This is data from a Normal random variable.)
